...
A run is stored in two places. `*_runs.jsonl` holds scores, digests, configuration and per-case **metrics** — small, permanent, **tracked**. `*_runs_detail/` holds the bulky per-case detail for the most recent 5 runs only and is **gitignored**. A retrieval detail is **self-contained evidence**, not just a verdict: the question and expectations as they stood for that run, the answer, and every passage retrieved **in order** with its text — because a past failure has to be diagnosable when the index, model and corpus have all moved on, and reranking is judged on order. Ceilings (`MAX_PASSAGE_CHARS`, `MAX_ANSWER_CHARS`) sit above anything this corpus produces, so nothing is clipped today; when one fires it is recorded as `<field>_truncated_from`, never silently. ~150KB per run. The split works because a baseline is only ever read through its metrics and case ids; detail is read from the current run alone. Read a kept run's detail with `--detail [RUN_ID]`; an older one says so rather than failing silently.
